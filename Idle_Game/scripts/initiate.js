...
function build_object(trees2, stone, research, green,) {
    let mult = 1;
    let owned = 0;
    let obj = Object({trees2, stone, research, green, mult, owned});
    return obj
}
...
b_empty = build_object(0, 0, 0, 0);
b_click_costs = build_object(10, 5, 0, 0);
b_prod_costs = build_object(100, 10, 0, 0);
b_research_costs = build_object(1000, 25, 0, 0);
b_costdown_costs = build_object(2500, 50, 0, 0);
b_soul_costs = build_object(5000, 75, 0, 0);
b_stone_costs = build_object(7500, 100, 0, 0);
b_shop_costs = build_object(10000, 1000, 0, 0);
b_school_costs = build_object(12500, 100, 0, 0);
b_bush_costs = build_object(1000, 25, 0, 0);
b_plantation_costs = build_object(1000, 25, 0, 0);
b_num_total = 0;
...
buildings_data = Object({b_click_costs, b_prod_costs, b_research_costs, b_costdown_costs, b_num_total});
buildings_data_base = JSON.parse(JSON.stringify(clone_object(buildings_data)));

//Other buildings variables
let cant_buy_building = 0;
...
game_save = Object({trees, manual_power, trees_chopped, tree_levels, lifetime_clicks, lifetime_trees_chopped, lifetime_buildings_bought, land, buildings_data});
game_save_base = JSON.parse(JSON.stringify(Object({trees, manual_power, trees_chopped, tree_levels, lifetime_clicks, lifetime_trees_chopped, lifetime_buildings_bought, land, buildings_data})));
